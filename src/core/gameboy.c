...
GameBoy * gameboy_create() {
    return calloc(1, sizeof(GameBoy));
}
...
void gameboy_insert_cartridge(GameBoy * const gb, Cartridge * const cart) {
    gb->cartridge = cart;
}
...
void gameboy_initialize(GameBoy * const gb, bool skip_bootrom) {
    ...
    dma_initialize(gb);
    interrupt_controller_initialize(gb, skip_bootrom);
    joypad_initialize(gb);
    ppu_initialize(gb);
    processor_initialize(gb, skip_bootrom);
    serial_initialize(gb);
    sound_controller_initialize(gb);
    timer_initialize(gb, skip_bootrom);
}
